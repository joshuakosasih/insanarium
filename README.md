...
**Income bubbles** appear every 3–5 active seconds, including at $0 with no fish. Small unlabelled bubbles rise from near the bottom to the surface. Pop one for a random $0.50, $1, $2, or $3; the reward is revealed after popping. Up to three float in the tank at once and disappear at the surface (roughly 22–24 seconds); missed bubbles give no money. A purchased Bubble Puffer wanders through the water and may chase each bubble it notices. It pops accepted targets on contact and briefly inflates afterward. Bubbles are separate from fish rewards and are not generated offline, so the puffer earns no offline bubble income. Wallet arithmetic uses integer cents, retaining fractional dollars through purchases, saves, backups, and offline care. Bubble reward amounts and a multiplier are isolated in `scripts/systems/bubble_rewards.gd` for future upgrades; no bubble upgrade is sold yet. A replacement or third fish costs $50; each additional living fish raises the next shop price by about 65%, rounded to the nearest $5. The price reaches $2,745 at ten fish and $249,000 at nineteen, making breeding and selling stock central near the capacity limit. Losing or selling every fish does not reset the tank, upgrades, or pets: earn money and rebuild.

| Stage | Requirement | Coin payout every 20 seconds | Normal sale | Mutated sale |
| --- | --- | --- | --- | --- |
| Baby | Starting stage | $1 bronze | $15 | $75 |
| Adult | 10 growth credits | $2 silver | $120 | $600 |
| Royal | 30 growth credits | $3 gold | $350 | $1,750 |
| Diamond | 75 actual meals | $10 blue diamond | $1,200 | $6,000 |

Existing saves keep their fish counts and earned growth stages; the new thresholds apply to future advancement. Every normal fish is amber, including normal Diamond fish. Each growth-stage transition has an **8% mutation chance** for an unmutated fish. A mutation changes its color to Azure, Rose, or Jade and doubles its sale value. Each fish can mutate only once; mutation does not increase coin production. Already purchased or starting babies do not randomly start mutated.

## Feed and survival

Basic costs $2 per pellet and grants 1 growth credit. Upgrade once for $100 to Premium ($4, 2 credits), then for $250 to Deluxe ($6, 3 credits). Manual feeding always uses the highest purchased tier: there is no downgrade selector. All food restores 0.65 hunger, and Diamond still requires 75 actual meals.
...
While the game is focused, hunger grows at the relaxed rate of **1/120 per second** (two minutes from fully fed to maximum hunger), and the red bar gives **45 seconds** at maximum hunger to rescue the fish. When the game loses focus, is minimized, or its web tab is hidden, live simulation pauses. On return, a bounded data-only calculation advances care at **10%** speed, up to eight real hours away. Returning immediately restores active speed without resetting hunger or starvation progress. Feeding resets the starvation timer. Dead fish stop participating immediately, flip belly-up, float to the surface, then fade. An orange dot means hungry; red means starving.
...
Save schema 2 migrates existing schema 1 files at the same local path. Legacy fish receive unique IDs, but prior age, birth time, and ancestry are explicitly unknown rather than guessed; age thereafter is labeled as tracked time. Fish saved before Vitality and Swim Speed were introduced receive neutral values of 100 maximum health and standard species speed. Existing wallet, earlier traits, growth, and automation remain intact. The saved ID counter prevents reuse across sessions.
...
Catch-up covers hunger, starvation, food expiry, stocked feeding, seahorse food, growth/mutation, fish age and old-age death, reward production, and snail collection. It uses at most 2,880 one-second care steps (eight real hours at 10%), without running movement or physics. Hungry fish receive available food by need. Offline snail collections are estimated from its speed, movement stamina, and sleep cycle, with the oldest rewards expiring first. These are approximations, not a replay of swimming paths. **Breeding and alien attacks do not occur offline.** Time beyond the cap is discarded. Returning after 30 seconds or more shows a summary of care, earnings, growth, and losses.

## Tank Care panel

Click **Tank care** above the aquarium for live hunger/stock/population warnings and an eight-hour away forecast. It shows reserve exhaustion, first starvation risk, and estimated feeding demand versus available automation. Predictions use the same offline care model without changing the tank, money, or random growth outcomes. They refresh every ten seconds while open and after economy changes; immediate care warnings refresh four times per second.
...
## Optional alien challenges

Lethal alien encounters are **off by default**. Enable **Alien challenges** to opt in. Random delays are 90–150 seconds, followed by a 5-second warning at the entry point. An alien chases at 90 pixels/second and kills fish on contact, at most once per 2 seconds. Eight clicks defeat it for a $10 diamond. Click its left side to push it right (and vice versa); there is no stun. Turning challenges off removes an active alien without a reward. Challenges are disabled again when reopening a saved tank.
...
- `scripts/systems/idle_assets.gd`: owned pets/machine and stocked pellets.
...
Relaxed opening verification: deterministic attentive care reaches Adult at 510 active seconds on Basic food. Two fish consume $44 of Basic food over ten minutes. This excludes swimming delays and income; native audio tests verify playback/mute, and browser checks verify bubble rendering and mute persistence.
