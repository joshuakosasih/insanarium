...
Open **Shop** above the tank to browse reusable illustrated cards for fish, helpers, feeder stock, and feed upgrades. The current wallet balance stays visible in the shop header. Selecting a card opens its details and purchase action. Dragging directly over cards scrolls the catalog on touch screens and with a held mouse. Feed and stock previews adopt the currently unlocked pellet color: orange Basic, blue Premium, or violet Deluxe. The shop confirms each successful purchase; purchases also play the synthesized buy sound when sound is enabled. Paired reward upgrades require the matching lifetime or capacity level before their value multiplier, so the disabled button explains the exact prerequisite. Away Time starts unlocked, then requires the Auto-feeder for 30 minutes, the Seahorse for two hours, and the completed 15-fish goal for eight hours.
