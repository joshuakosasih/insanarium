...
func run() -> void:
	...
	slow.fertility = PackedFloat32Array([0.0, 0.0])
	...
	fast.fertility = PackedFloat32Array([1.0, 1.0])
	...
	check(decoded.metabolism == fast.metabolism and decoded.allocation == fast.allocation and decoded.vitality == fast.vitality and decoded.speed == fast.speed and decoded.fertility == fast.fertility, "five-trait genome data round-trips exactly")
	var legacy_genome := FishGenome.new()
	legacy_genome.from_data({"metabolism": [0.2, 0.8], "allocation": [0.4, 0.6]})
	check(legacy_genome.vitality == PackedFloat32Array([0.5, 0.5]) and legacy_genome.speed == PackedFloat32Array([0.5, 0.5]) and legacy_genome.fertility == PackedFloat32Array([0.5, 0.5]), "older fish receive neutral newer traits")
