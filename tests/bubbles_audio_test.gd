...
func run() -> void:
	...
	tank.audio.set_music_muted(true)
	tank.audio.play("bubble")
	check(tank.audio.voices.any(func(v: AudioStreamPlayer) -> bool: return v.playing) and not tank.audio.calm_music.playing and not tank.audio.alien_music.playing, "music can be muted while effects keep playing")
	for voice in tank.audio.voices:
		voice.stop()
	tank.audio.set_effects_muted(true)
	tank.audio.play("coin")
	check(tank.audio.voices.all(func(v: AudioStreamPlayer) -> bool: return not v.playing), "effects can be muted independently")
	tank.audio.set_music_muted(false)
	tank.audio.set_danger_music(false)
	tank.audio.set_effects_muted(false)
	...
	tank.audio.set_backgrounded(true)
	check(not tank.audio.calm_music.playing and not tank.audio.alien_music.playing and tank.audio.voices.all(func(v: AudioStreamPlayer) -> bool: return not v.playing), "backgrounding stops both music and current effects immediately")
	tank.audio.set_music_muted(false)
	check(not tank.audio.calm_music.playing, "saved Music preference cannot restart audio while backgrounded")
	tank.audio.set_backgrounded(false)
	check(tank.audio.calm_music.playing, "returning resumes music according to the saved preference")
	check(tank.audio.effects.size() == 8 and tank.audio.effects.bubble.data.size() > 0 and tank.audio.effects.bubble.data != tank.audio.effects.coin.data, "bubble pop and coin chime contain distinct PCM audio")
	tank.audio.set_music_muted(true)
	tank.audio.set_effects_muted(true)
